...
{
    csv_find_row(row, "cardno")->value.l = account->cardno;
...
personal_read(char *fname, struct account *account)
{
    ...
    account->cardno = csv_find_row(row, "cardno")->value.l;
...
account_read(long int cardno, struct account **account_return)
{
    ...
    sprintf(base, DATA_PREFIX"%ld\\"FPERSONAL, cardno);
    account->fpersonal = strdup(base);
    sprintf(base, DATA_PREFIX"%ld\\"FFREEZE, cardno);
    account->ffreeze = strdup(base);
    sprintf(base, DATA_PREFIX"%ld\\"FCONSUME_RECORD, cardno);
    account->fconsume_record = strdup(base);
    sprintf(base, DATA_PREFIX"%ld\\"FCONSUME_UNDO, cardno);
